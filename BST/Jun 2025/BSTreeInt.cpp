...
int BSTreeInt::deleteLeafs()
{
	return deleteSingleLeafParents(root);
}

int BSTreeInt::deleteSingleLeafParents(BSTNodeInt* root)
{
	return deleteParents(this->root);
}

int BSTreeInt::deleteParents(BSTNodeInt*& root)
{
	if (root == nullptr)
		return 0;
	...
	return deleteParents(root->left) + deleteParents(root->right);
}