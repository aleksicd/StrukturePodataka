...
{
	int maxDepth = -1;
	BSTNodeInt* deepest = nullptr;
	return findDeepest(root, 0, deepest, maxDepth);
}

BSTNodeInt* BSTreeInt::findDeepest(BSTNodeInt* root, int currDepth, BSTNodeInt*& deepest, int& maxDepth)
{
	if (root == nullptr)
		return deepest;

	if ((root->left != nullptr || root->right != nullptr) && root->key % 2 == 0 && currDepth > maxDepth)
	{
		deepest = root;
		maxDepth = currDepth;
	}

	findDeepest(root->left, currDepth + 1, deepest, maxDepth);
	findDeepest(root->right, currDepth + 1, deepest, maxDepth);

	return deepest;
}

BSTNodeInt* BSTreeInt::getDeepest()
{
	return getDeepestEvenParent(root);
}