...
int BSTreeInt::saberiSve(int low, int high)
{
	return saberiSveRekurzija(root, low, high);
}

int BSTreeInt::saberiSveRekurzija(BSTNodeInt* ptr, int low, int high)
{
	BSTNodeInt* curr = ptr;
	if (curr == nullptr)
		return 0;
	if (curr->key >= low && curr->key <= high)
		return curr->key + saberiSveRekurzija(curr->left, low, high) + saberiSveRekurzija(curr->right, low, high);
	if (curr->key < low)
		return saberiSveRekurzija(curr->right, low, high);
	if (curr->key > high)
		return saberiSveRekurzija(curr->left, low, high);
}