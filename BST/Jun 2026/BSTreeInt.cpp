...
void BSTreeInt::zameni()
{
	BSTNodeInt* prev = nullptr;
	BSTNodeInt* prvi = nullptr;
	BSTNodeInt* drugi = nullptr;

	zameniCvorove(root, prev, prvi, drugi);
...
void BSTreeInt::zameniCvorove(BSTNodeInt* ptr, BSTNodeInt*& prev, BSTNodeInt*& prvi, BSTNodeInt*& drugi)
{
	BSTNodeInt* curr = ptr;
	if (curr == nullptr)
		return;
	
	zameniCvorove(curr->left, prev, prvi, drugi);
	if (prev != nullptr && prev->key > curr->key)
	{
		if (prvi == nullptr)
		{
			prvi = prev;
	...
	zameniCvorove(curr->right, prev, prvi, drugi);
	if (prev != nullptr && prev->key > curr->key)
	{
		if (prvi == nullptr)
		{
